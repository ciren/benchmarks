...
object Benchmarks {
  ...
  def brad[A: Field](x: Dimension3[A]) = {
    ...
    y.zipWithIndex mapSum { case (yi, i) =>
      val ui = i + 1
      val vi = 16 - ui
      val wi = min(ui, vi)
  ...
  def griewank[N<:Nat, A: Field : NRoot : Trig](x: Dimension[N,A]) = {
    val prod = x.zipWithIndex.mapProduct { case (xi, i) =>
      cos(xi / sqrt(i + 1.0))
    }
  ...
  def hartman3[A: Field : Trig](x: Dimension3[A]) = {
    ...
    -(a zip c zip p).mapSum { case ((ai, ci), pi) =>
      val power = (ai zip pi zip x) mapSum { case ((aij, pij), xj) =>
        aij * ((xj - pij) ** 2)
      }
      ci * exp(-power)
    }
  ...
  def michalewicz[N<:Nat, A: Field : IsReal : NRoot : Trig](m: Double = 10.0)(x: Dimension[N,A]) =
    -x.zipWithIndex.mapSum { case (xi, i) =>
  ...
  def watson[A: Field](x: Dimension6[A]) = {
    ...
    val t1 = (0 to 29).mapSum { i =>
      val ai = i / 29.0
      val sum4 = x.toList.tail.zipWithIndex.mapSum { case (xj, j) =>
        (j + 1) * (ai ** j.toDouble) * xj
      }
      val sum5 = x.zipWithIndex.mapSum { case (xk, k) =>
        (ai ** k.toDouble) * xk
      }
      (sum4 - (sum5 ** 2) - 1) ** 2
    }
