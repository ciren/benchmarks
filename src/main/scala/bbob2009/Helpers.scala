...
import shapeless._
import spire.algebra.{Order,Field,NRoot,Signed,Trig}
...
object Helpers {
  ...
  def asymmetric[N<:Nat,A:Field:Order:NRoot](β: Double):
    Dimension[N,A] => Dimension[N,A] =
      x => {
        val D = x.size
        x.zipWithIndex map { case (xi, i) =>
          if (xi <= 0) xi
          else {
            val exponent =
